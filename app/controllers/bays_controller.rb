class BaysController < ApplicationController
  ...
  # GET /bays.json
  def index
    @bays = Bay.find_by_sql("SELECT b.id, p.name, b.code, b.position FROM products AS p INNER JOIN bays AS b ON p.bay_id=b.id ORDER BY position;")
  end
  ...
  private
    ...
    def bay_params
      params.require(:bay).permit(:code, :position)
    end
end
